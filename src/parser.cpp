...
*/
    ...
    auto Parser::editing(std::stringstream configuration){
        ...
        /* Iterate over the tags values */

        for (json::iterator it = j.begin(); it != j.end(); ++it) {
           
            /* Access nested elements */
            for (auto& x : it.value().items())
            {
                /* Create a new json object to access sub-elements */
                json j2 = x;

                std::cout<< j2["checks"] <<std::endl;
            
                /* Access edit operators and call the corresponding method */

                for (const auto& el : j2["checks"]){
